...
export const send = action({
  ...
  async handler(ctx, { teamId, email, roleId }) {
    const { inviterEmail, teamName } = await ctx.runMutation(
      internal.team.invites.prepare,
      { teamId },
    );
    const inviteId = await ctx.runMutation(internal.team.invites.create, {
      teamId,
      email,
      roleId,
      inviterEmail,
    ...
    try {
      ...
      await ctx.runMutation(api.team.invites.deleteInvite, {
        inviteId,
      });
      throw error;
    }
...
async function sendInviteEmail({
  ...
  const { error } = await resend.emails.send(
    InviteEmail({ inviteId, inviterEmail, teamName, email }),
  );
